...
Use the **Share** button in the bottom navigation and choose a share type. **Gist** publishes the selected tab as a public GitHub Gist, then lets you copy or use the native share sheet for the generated link. **GitHub** commits the selected current version to `monperrus/mws_apps`. **Copy’n’paste** copies the complete HTML for the Paste flow. **PWA** sharing is not implemented yet. Publishing needs a GitHub fine-grained personal access token with **Gists: read and write** permission; GitHub commits need **Contents: read and write** access. The token is stored only in the browser's local storage so the field can be prefilled for future publishes.
